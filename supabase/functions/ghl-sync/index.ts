...
type Person = { id?: string; name?: string; phone?: string; email?: string; status?: string }
...
Deno.serve(async (req) => {
  ...
  // 'consumers' | 'attendants' | undefined (both)
  ...
  const { data: rows, error: readErr } = await admin
    ...
    .in('data_key', ['consumers', 'attendants'])
  if (readErr) return json({ error: 'Could not read hub data: ' + readErr.message }, 500)

  const byKey: Record<string, Person[]> = { consumers: [], attendants: [] }
  for (const r of rows ?? []) {
    if (Array.isArray(r.data_value)) byKey[r.data_key] = r.data_value as Person[]
  }

  const plan: { key: string; tag: string }[] = [
    { key: 'consumers', tag: 'consumer' },
    { key: 'attendants', tag: 'attendant' },
  ].filter((p) => !only || p.key === only)

  const report: Record<string, { total: number; synced: number; skipped: Outcome[]; failed: Outcome[] }> = {}

  for (const { key, tag } of plan) {
    const people = byKey[key] ?? []
    const results = await inBatches(people, 3, (p) => upsertContact(p, tag, headers, locationId))
    report[key] = {
      total: people.length,
  ...
  // Record what happened under a key of our own. Never touches 'consumers' or
  // 'attendants', so this cannot collide with an edit happening in the hub.
